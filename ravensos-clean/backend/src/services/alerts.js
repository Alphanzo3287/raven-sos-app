...
export function createAlertService(store, hub) {
  async function trigger({ ownerId, lat, lng, accuracyM = null, address = null, triggerType = TriggerType.MANUAL, isSilent = false, idempotencyKey = null }) {
    ...
    const message = buildAlertMessage({ userName: owner.displayName, address, lat, lng });
