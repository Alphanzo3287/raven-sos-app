...
export function buildAlertMessage({ userName, address, lat, lng, watchLink = null }) {
  const map = config.mapLinks.google(lat, lng);
  const link = watchLink || map; // prefer the live-tracking link; fall back to a map pin
  const title = `${userName} triggered an emergency alert`;
  const body = `${userName} needs help near ${address ?? 'an unknown location'}. Tap to see their live location.`;
  const sms = `EMERGENCY: ${userName} triggered a Raven SOS alert${address ? ` near ${address}` : ''}.\nSee their live location: ${link}`;
  return { title, body, sms, map, link };
}
